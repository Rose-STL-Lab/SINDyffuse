    ...
    if path.is_dir():
        for name in ('latest.pt', 'best.pt'):
            ...
            if candidate.is_file():
                return candidate
    raise FileNotFoundError(f'Missing activation surrogate checkpoint at {path} (expected file or latest.pt in directory)')

class ActivationSurrogateGuidance(nn.Module):

    def __init__(self, model: nn.Module, *, mean_q: Optional[torch.Tensor]=None, std_q: Optional[torch.Tensor]=None):
        super().__init__()
        self.model = model
    ...
    def _normalize_q(self, q: torch.Tensor) -> torch.Tensor:
        ...
        std = self.std_q.view(1, 1, -1).clamp_min(1e-08)
        return (q - mean) / std

    def predict_activations(self, q: torch.Tensor) -> torch.Tensor:
        return self.model(self._normalize_q(q))

def load_activation_surrogate_guidance(checkpoint: str | Path, *, device: torch.device | str='cpu', data_root: str | None=None) -> ActivationSurrogateGuidance:
    path = _resolve_checkpoint_path(checkpoint)
    try:
        payload: Dict[str, Any] = torch.load(path, map_location='cpu', weights_only=False)
    except TypeError:
        payload = torch.load(path, map_location='cpu')
    model = build_activation_surrogate(model_type=str(payload.get('model_type', 'mlp')), input_dim=int(payload['input_dim']), output_dim=int(payload['output_dim']), hidden_dim=int(payload.get('hidden_dim', 256)), num_layers=int(payload.get('num_layers', 3)), dropout=float(payload.get('dropout', 0.1)), num_heads=int(payload.get('num_heads', 4)), dim_feedforward=int(payload.get('dim_feedforward', 128)), max_seq_len=int(payload.get('window_size', 196)))
    ...
    wrapper = ActivationSurrogateGuidance(model.to(dev), mean_q=mean_q, std_q=std_q)
