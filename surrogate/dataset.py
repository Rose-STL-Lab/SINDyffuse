...
from nimble.b3d_io import b3d_has_muscle_activations, b3d_has_sindyffuse_custom_values, read_muscle_activations_frames
from nimble.gap_utils import is_nan_placeholder_activations, motion_has_valid_activations, read_activation_validity_mask_frames, window_is_valid
from nimble.muscle_b3d import is_zero_placeholder_activations

class ActivationB3DDataset(Dataset):

    def __init__(self, data_root: str, *, split: str='train', window_size: int=64, window_stride: int=16, normalize_q: bool=True, max_motions: int=0, skip_zero_placeholders: bool=True, skip_invalid_activations: bool=True, zero_atol: float=1e-08, min_valid_fraction: float=0.95):
        ...
        if self.normalize_q:
            ...
            if not mean_p.is_file() or not std_p.is_file():
                raise FileNotFoundError(f'Nimble Q stats missing under {self.b3d_dir}. Run scripts/compute_normalization.py (writes Mean.npy / Std.npy).')
        ...
        for sid in ids:
            ...
            if tlen < self.window_size:
                self.num_motions_skipped_other += 1
                continue
            if self.skip_zero_placeholders or self.skip_invalid_activations:
                ...
                if self.skip_invalid_activations and is_nan_placeholder_activations(act):
                    ...
                    continue
                if self.skip_invalid_activations and (not motion_has_valid_activations(subj, 0, tlen)):
                    self.num_motions_skipped_nan += 1
                    continue
            ...
            for st in range(0, tlen - self.window_size + 1, self.window_stride):
                if self.skip_invalid_activations:
                    act_win = read_muscle_activations_frames(subj, 0, st, self.window_size)
                    try:
                        mask_win = read_activation_validity_mask_frames(subj, 0, st, self.window_size)
                    except Exception:
                        mask_win = np.ones((self.window_size,), dtype=np.float32)
