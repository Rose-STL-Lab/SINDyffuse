...
def _process_one_moco(item: tuple) -> dict:
    sid, out_root_s, skip_existing, verbose_log_path, act_cfg_json, gate_cfg_path, ik_status, ik_stats_json = item
    if verbose_log_path:
    ...
    try:
        with opensim_quiet(act_cfg.opensim_log_level):
            stats, num_dofs, meta_strings, manifest_status = patch_b3d_moco(out_b3d, trial_name=sid, act_cfg=act_cfg, gate_cfg=gate_cfg, ik_manifest_status=ik_status, ik_stats=ik_stats)
    ...
    if manifest_status == 'moco_skipped':
        row['moco_skipped_reason'] = stats.get('moco_skipped_reason') or stats.get('ik_gate_reason') or 'preflight gate'
    if meta_strings:
...
def run_preprocess_moco(args: argparse.Namespace, logger) -> None:
    ...
    for sid in ids:
        ...
        work.append((sid, str(out_root), bool(args.skip_existing), verbose, act_cfg_json, gate_cfg_path, ik_status, ik_stats_json))
