...
def train_activation_surrogate(*, data_root: str, output: str, split: str='train', val_split: str='val', window_size: int=64, window_stride: int=16, normalize_q: bool=True, max_motions: int=0, skip_zero_placeholders: bool=True, model_type: str='mlp', hidden_dim: int=256, num_layers: int=3, dropout: float=0.1, num_heads: int=4, dim_feedforward: int=128, lr: float=0.001, weight_decay: float=0.0001, epochs: int=50, batch_size: int=32, lambda_temporal: float=0.1, device_name: str='auto', num_workers: int=0, seed: int=42, distributed_cfg: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
    ...
    log_main(f'[activation_surrogate] device={device} world_size={get_world_size()} distributed={use_ddp} per_gpu_batch={per_gpu_batch} global_batch={per_gpu_batch * get_world_size()}')
    out_dir = Path(output)
    if is_main_process():
    ...
    def _save_checkpoint(path: Path, *, epoch: int, val_loss: float) -> None:
        if not is_main_process():
            return
        torch.save({**ckpt_payload, 'model_state_dict': model_state_dict(model), 'epoch': int(epoch), 'val_loss': float(val_loss)}, path)
    for epoch in range(int(epochs)):
        ...
        history.append({'epoch': float(epoch), 'train_loss': train_loss, 'val_loss': val_loss, 'val_muscle_l1_mean': val_muscle_l1_mean, 'val_muscle_l1_max': val_muscle_l1_max})
        if is_main_process():
            logger.progress(f'epoch {epoch + 1}/{epochs} train={train_loss:.6f} val={val_loss:.6f} val_muscle_l1_mean={val_muscle_l1_mean:.6f} val_muscle_l1_max={val_muscle_l1_max:.6f}')
            logger.verbose(f'[activation_surrogate] epoch {epoch + 1}/{epochs} train={train_loss:.6f} val={val_loss:.6f}')
    if is_main_process():
        _save_checkpoint(latest_path, epoch=int(epochs) - 1, val_loss=history[-1]['val_loss'] if history else float('nan'))
...
def main() -> None:
    ...
    parser.add_argument('--config', default='', help='JSON config (default: configs/train_surrogate.json)')
    ...
    parser.add_argument('--skip_zero_placeholders', type=int, default=1, help='1=drop motions whose muscle_activations are all-zero fallbacks')
    ...
    args.output = str(resolve_run_dir(str(args.output).strip() or out_override or None, family='activation_surrogate'))
