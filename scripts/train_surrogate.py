...
def _build_surrogate_dataset(data_root: str, **kwargs):
...
def train_activation_surrogate(
    *,
    ...
    ds_kwargs = dict(
        ...
        skip_unlabeled=skip_zero_placeholders,
        skip_zero_placeholders=skip_zero_placeholders,
    )

    train_ds = _build_surrogate_dataset(data_root, split=split, **ds_kwargs)
    try:
        val_ds = _build_surrogate_dataset(data_root, split=val_split, **ds_kwargs)
    except ValueError:
        val_ds = None
