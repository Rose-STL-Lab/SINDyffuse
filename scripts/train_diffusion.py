...
def _prepare_diffusion_config(config_path: Path, *, guidance: str='', data_root: str='') -> dict:
...
def train(config_path: str, out_dir: str, *, preload: bool=False) -> None:
    ...
    if is_main_process() and _nw != _nw_requested:
        log_main(f'[train] num_workers={_nw_requested} disabled on CUDA (fork-after-GPU init crashes); using {_nw}. Set train.allow_dataloader_fork_after_cuda=true with spawn for multi-worker loading.')
    train_sampler = None
    if use_ddp:
        train_sampler = DistributedSampler(train_ds, num_replicas=get_world_size(), rank=get_rank(), shuffle=True, drop_last=True)
    train_loader = DataLoader(train_ds, batch_size=per_gpu_batch, shuffle=train_sampler is None, sampler=train_sampler, num_workers=_nw, drop_last=True, collate_fn=_collate, pin_memory=device.type == 'cuda')
    ...
    model = DiffusionTransformer(input_dim=int(train_ds.feature_dim), model_dim=int(model_cfg.get('model_dim', 512)), num_layers=int(model_cfg.get('num_layers', 8)), num_heads=int(model_cfg.get('num_heads', 8)), dropout=float(model_cfg.get('dropout', 0.1)), max_seq_len=int(data_cfg.get('window_size', 64)), clip_dim=int(model_cfg.get('clip_dim', 512))).to(device)
    ...
    opt = torch.optim.AdamW(model.parameters(), lr=float(train_cfg.get('lr', 0.0002)), weight_decay=float(train_cfg.get('weight_decay', 0.01)))
    ...
    while step < max_steps:
        ...
        for motion, captions, _motion_ids in train_loader:
            ...
            guide_stats = {}
            denom = torch.clamp(sqrt_ab, min=1e-08)
        ...
        epoch += 1
    if is_main_process():
        torch.save({'model_state': model_state_dict(model), 'step': max_steps, 'feature_dim': int(train_ds.feature_dim)}, out / 'last.pt')
...
def main() -> None:
    ...
    cfg = _prepare_diffusion_config(config_path, guidance=str(args.guidance), data_root=str(args.data_root))
    guidance_mode = _validate_diffusion_inputs(cfg)
    out_dir = resolve_run_dir(str(args.out_dir).strip() or None, family='diffusion', guidance=guidance_mode.value)
    ...
    def _run(_logger: RunLogger) -> None:
        try:
            train(config_path=str(resolved_config), out_dir=str(out_dir), preload=bool(args.preload))
        finally:
            cleanup_distributed()
