...
from common.distributed import barrier, cleanup_distributed, get_rank, get_world_size, init_distributed, is_distributed, is_main_process, log_main, log_gpu_diagnostics, maybe_relaunch_with_torchrun, model_state_dict, parse_distributed_enabled, resolve_nproc_per_node, should_auto_relaunch_torchrun, resolve_train_device, seed_all, setup_spawn_if_distributed, shard_indices, unwrap_module as unwrap_train_module, wrap_ddp
from common.paths import default_humanml3d_root, humanml3d_text_dir, nimble_b3d_dir, sindy_latest_link, update_latest_symlink
...
def _build_lr_scheduler(opt: optim.Optimizer, *, scheduler_name: str, epochs: int, warmup_epochs: int, lr_min: float):
    ...
    if name == 'plateau':
        return optim.lr_scheduler.ReduceLROnPlateau(opt, mode='min', factor=0.5, patience=max(5, total // 20), min_lr=eta_min)
...
def _caption_for_window(sample_id: str, text_dir: Path, fps: float, window_size: int, cache: Dict[str, List[Tuple[str, int, int]]]) -> str:
...
@torch.no_grad()
def _text_embed(captions: List[str], device: torch.device, model_name: str='ViT-B/32', batch_size: int=128) -> np.ndarray:
...
def _run_training_loop(*, model: TextToXi, opt: optim.Optimizer, scheduler: Any, device: torch.device, length: int, tr_idx: np.ndarray, val_idx: np.ndarray, captions: List[str], cap_to_idx: Dict[str, int], emb_t: torch.Tensor, epochs: int, batch_size: int, lambda_l1: float, lambda_xi_smooth: float, grad_clip: float, early_stop_patience: int, rng: np.random.Generator, fetch_batch: Any, run_validation: bool=True, n_bio_targets: int=N_BIO_TARGETS, target_weights: torch.Tensor | None=None) -> Tuple[float, Optional[Dict[str, torch.Tensor]], Dict[str, float], int]:
    ...
    for ep in range(1, int(epochs) + 1):
        ...
        should_stop = 0
        if is_main_process() and int(early_stop_patience) > 0 and (len(val_idx) > 0) and (stale_epochs >= int(early_stop_patience)):
            should_stop = 1
            from common.run_logging import get_run_logger
            get_run_logger().progress(f'early_stop at epoch={ep} (patience={int(early_stop_patience)}, best_val_loss={best_val:.6f})')
        if is_distributed():
        ...
        if (ep % max(1, epochs // 20) == 0 or ep == 1) and is_main_process():
            ...
            get_run_logger().progress(f'epoch={ep}/{epochs} lr={lr_now:.2e} train_loss={np.mean(losses):.6f} val_loss={vl:.6f} val_bio_mse={val_bio:.6f} val_muscle_mse={val_muscle:.6f}')
    best_metrics['epochs_run'] = float(last_epoch)
    return (best_val, best_state, best_metrics)

def train(*, data_root: str, split: str, fps: float, output: str, window_size: int, window_stride: int, theta_tier: str, include_u: bool, include_c: bool, max_samples: int, lr: float, lr_min: float, warmup_epochs: int, scheduler_name: str, weight_decay: float, grad_clip: float, early_stop_patience: int, epochs: int, batch_size: int, lambda_l1: float, lambda_xi_smooth: float, hidden_dim: int, num_layers: int, dropout: float, ff_mult: int, num_experts: int, fallback_caption_weight: float, clip_model_name: str, clip_text_batch_size: int, device_name: str, bio_log_every: int, preload: bool=False, skip_zero_placeholders: bool=True, zero_atol: float=1e-08, target_weights: Sequence[float] | None=None, distributed_cfg: Optional[Dict[str, Any]]=None, seed: int=42) -> Dict[str, Any]:
    ...
    if preload:
        ...
        u, c, y, u_names, c_names, sample_ids = collect_windows(data_root, split, fps, window_size, window_stride, max_samples, log_every=bio_log_every, skip_zero_placeholders=skip_zero_placeholders, zero_atol=zero_atol)
        n, t, _u_dim = u.shape
        ...
        theta_flat, feature_names = ThetaLibrary(spec=spec).build(u=u_in, c=c_in, u_names=u_names if include_u else [], c_names=c_names if include_c else [])
        length = t - 1
        ...
        index, theta_scaler, y_scaler, u_names, c_names, feature_names, target_dim, spec = prepare_lazy_sindy_data(data_root, split, fps=fps, window_size=window_size, window_stride=window_stride, max_samples=max_samples, theta_tier=theta_tier, include_u=include_u, include_c=include_c, log_every=bio_log_every, skip_zero_placeholders=skip_zero_placeholders, zero_atol=zero_atol)
        ...
        n = len(sample_ids)
        lazy_dataset = SindyWindowDataset(data_root, index, window_size=window_size, fps=fps, theta_spec=spec, include_u=include_u, include_c=include_c, theta_scaler=theta_scaler, y_scaler=y_scaler, u_names=u_names, c_names=c_names, feature_names=feature_names, target_dim=target_dim)
        theta_s = None
        y_s = None
    text_dir = humanml3d_text_dir(data_root)
    caption_cache: Dict[str, List[Tuple[str, int, int]]] = {}
    captions = [_caption_for_window(str(sid), text_dir, fps, window_size, caption_cache) for sid in sample_ids.tolist()]
    ...
    model = TextToXi(in_dim=unique_emb.shape[1], hidden_dim=int(hidden_dim), theta_dim=f, target_dim=target_dim, num_experts=int(num_experts), max_seq_len=int(length), num_layers=int(num_layers), dropout=float(dropout), ff_mult=int(ff_mult)).to(device)
    ...
    scheduler = _build_lr_scheduler(opt, scheduler_name=str(scheduler_name), epochs=int(epochs), warmup_epochs=int(warmup_epochs), lr_min=float(lr_min))
    log_main(f'[sindy/train] device={device} world_size={get_world_size()} distributed={use_ddp} train_windows={len(tr_idx)} per_gpu_batch={batch_size} target_dim={target_dim} (bio={N_BIO_TARGETS} muscle={N_MUSCLE_TARGETS}) layers={num_layers} dropout={dropout} scheduler={scheduler_name} weight_decay={weight_decay}')
    ...
    if preload:
        ...
        def fetch_batch(indices: List[int] | np.ndarray) -> Tuple[torch.Tensor, torch.Tensor]:
            ...
            return (theta_b, y_b)
    best_val, best_state, best_val_metrics = _run_training_loop(model=model, opt=opt, scheduler=scheduler, device=device, length=length, tr_idx=tr_idx, val_idx=val_idx, captions=captions, cap_to_idx=cap_to_idx, emb_t=emb_t, epochs=epochs, batch_size=batch_size, lambda_l1=lambda_l1, lambda_xi_smooth=lambda_xi_smooth, grad_clip=grad_clip, early_stop_patience=early_stop_patience, rng=rng, fetch_batch=fetch_batch, run_validation=is_main_process(), n_bio_targets=N_BIO_TARGETS, target_weights=target_w)
    ...
    if not is_main_process():
        return {'config': {'distributed_rank': int(get_world_size())}, 'metrics': {'best_val_loss': float(best_val)}}
    torch.save({'model_state': model_state_dict(model), 'in_dim': int(unique_emb.shape[1]), 'hidden_dim': int(hidden_dim), 'theta_dim': f, 'target_dim': target_dim, 'num_experts': int(num_experts), 'max_seq_len': int(length), 'num_layers': int(num_layers), 'dropout': float(dropout), 'ff_mult': int(ff_mult), 'bio_channel_names': bio_keys, 'muscle_channel_names': muscle_keys, 'target_channel_names': target_keys}, out / 'text_to_xi.pt')
    ...
    results = {'config': {'data_root': data_root, 'split': split, 'fps': float(fps), 'target_dim': target_dim, 'bio_channel_names': bio_keys, 'muscle_channel_names': muscle_keys, 'target_channel_names': target_keys, 'window_size': int(window_size), 'window_stride': int(window_stride), 'theta_tier': theta_tier, 'preload': bool(preload), 'hidden_dim': int(hidden_dim), 'num_layers': int(num_layers), 'dropout': float(dropout), 'ff_mult': int(ff_mult), 'lr': float(lr), 'lr_min': float(lr_min), 'warmup_epochs': int(warmup_epochs), 'scheduler': str(scheduler_name), 'weight_decay': float(weight_decay), 'grad_clip': float(grad_clip), 'early_stop_patience': int(early_stop_patience), 'lambda_l1': float(lambda_l1), 'lambda_xi_smooth': float(lambda_xi_smooth)}, 'metrics': {'best_val_loss': float(best_val), **best_val_metrics}}
...
def main() -> None:
    ...
    parser.add_argument('--config', default='', help='Path to train_sindy.json (default: configs/train_sindy.json next to repo root)')
    ...
    parser.add_argument('--lr_min', type=float, default=1e-06)
    ...
    parser.add_argument('--lambda_xi_smooth', type=float, default=0.0001)
    ...
    parser.add_argument('--zero_atol', type=float, default=1e-08)
    ...
    parser.add_argument('--preload', action='store_true', help='Load all B3D windows into RAM before training (default: read on demand)')
    ...
    args.output = str(resolve_run_dir(str(args.output).strip() or None, family='sindy'))
    ...
    def _run(_logger: RunLogger) -> None:
        try:
            metrics = train(data_root=args.data_root, split=args.split, fps=float(args.fps), output=args.output, window_size=int(args.window_size), window_stride=int(args.window_stride), theta_tier=args.theta_tier, include_u=bool(args.include_u), include_c=bool(args.include_c), max_samples=int(args.max_samples), lr=float(args.lr), lr_min=float(args.lr_min), warmup_epochs=int(args.warmup_epochs), scheduler_name=str(args.scheduler), weight_decay=float(args.weight_decay), grad_clip=float(args.grad_clip), early_stop_patience=int(args.early_stop_patience), epochs=int(args.epochs), batch_size=int(args.batch_size), lambda_l1=float(args.lambda_l1), lambda_xi_smooth=float(args.lambda_xi_smooth), hidden_dim=int(args.hidden_dim), num_layers=int(args.num_layers), dropout=float(args.dropout), ff_mult=int(args.ff_mult), num_experts=int(args.num_experts), fallback_caption_weight=float(args.fallback_caption_weight), clip_model_name=args.clip_model_name, clip_text_batch_size=int(args.clip_text_batch_size), device_name=args.device, bio_log_every=int(args.bio_log_every), preload=bool(args.preload), skip_zero_placeholders=bool(args.skip_zero_placeholders), zero_atol=float(args.zero_atol), target_weights=full_cfg.get('target_weights') if cfg_path else None, distributed_cfg=dist_cfg, seed=int(full_cfg.get('seed', 42)) if cfg_path else 42)
            if is_main_process():
    ...
    if args.no_run_log:
        ...
        with run_log_session(args.log_dir, script_name=script_name, argv=sys.argv, rank=rank) as (_paths, logger):
            if is_main_process():
