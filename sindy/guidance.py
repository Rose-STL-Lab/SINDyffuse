...
def _install_numpy_pickle_compat() -> None:
    try:
        import numpy._core as np_core_mod
    except Exception:
        np_core_mod = np.core
...
def _load_train_config(sild_dir: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        return {'theta_tier': str(cfg.get('theta_tier', defaults['theta_tier'])), 'include_u': bool(cfg.get('include_u', defaults['include_u'])), 'include_c': bool(cfg.get('include_c', defaults['include_c']))}
    except Exception:
        return dict(defaults)

class LearnedSINDyGuidance:

    def __init__(self, sild_dir: str, data_root: str, fps: float=20.0, clip_model_name: str='ViT-B/32', *, bio_physics: NimbleGuidanceConfig | None=None, surrogate_checkpoint: str | Path | None=None, target_weights: List[float] | None=None):
        ...
        if len(bio_names) != len(BIOMECH_COMPONENT_KEYS):
            raise ValueError(f'Checkpoint bio_channel_names length {len(bio_names)} != {len(BIOMECH_COMPONENT_KEYS)}; retrain with current SINDyffuse.')
        ...
        if len(muscle_names_ckpt) != N_MUSCLE_TARGETS:
            raise ValueError(f'Checkpoint muscle_channel_names length {len(muscle_names_ckpt)} != {N_MUSCLE_TARGETS}; retrain SINDy with muscle activation targets.')
        ...
        if self.target_dim != expected_target_dim:
            raise ValueError(f'Checkpoint target_dim={self.target_dim} != {expected_target_dim} (L_bio + 80 muscle); retrain SINDy on joint targets.')
        ...
        if not sur_path:
            raise ValueError('SINDy guidance with muscle targets requires surrogate_checkpoint (path to activation surrogate latest.pt).')
        self._surrogate: ActivationSurrogateGuidance = load_activation_surrogate_guidance(sur_path, data_root=str(self.data_root))
        ...
        self._theta_library = ThetaLibrary(spec=_make_theta_spec(str(tcfg['theta_tier']), self._theta_include_u, self._theta_include_c, self._theta_u_names))
    ...
    def _scale_theta(self, theta: torch.Tensor) -> torch.Tensor:
        ...
        return (theta - mean) / torch.clamp(scale, min=1e-08)
    ...
    def _scale_y(self, y: torch.Tensor) -> torch.Tensor:
        ...
        return (y - mean) / torch.clamp(scale, min=1e-08)
    ...
    def _actual_targets_from_motion(self, motion_norm: torch.Tensor) -> torch.Tensor:
        ...
        if muscle.shape[-1] != N_MUSCLE_TARGETS:
            raise ValueError(f'Surrogate returned {muscle.shape[-1]} muscle channels; expected {N_MUSCLE_TARGETS}')
    ...
    def loss_and_stats(self, motion_norm: torch.Tensor, captions: List[str], device: torch.device) -> tuple[torch.Tensor, Dict[str, float]]:
        ...
        return (loss, {'sindy_guidance_scalar': float(loss.detach().cpu().item()), 'sindy_bio_mse': bio_mse, 'sindy_muscle_mse': muscle_mse})