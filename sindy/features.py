...
def features_from_keypoints(keypoints: np.ndarray, fps: float) -> Tuple[np.ndarray, np.ndarray, List[str], List[str]]:
    ...
    cadence = np.full((t,), fill_value=1.0 / max(t * dt, 1e-06), dtype=np.float32)
    u_names = ['phase_sin', 'phase_cos', 'cadence_hz', 'root_speed_x', 'root_speed_y', 'root_speed_z', 'root_acc_x', 'root_acc_y', 'root_acc_z', 'pelvis_height']
    u = np.stack([np.sin(phase), np.cos(phase), cadence, root_v[:, 0], root_v[:, 1], root_v[:, 2], root_a[:, 0], root_a[:, 1], root_a[:, 2], root[:, 1]], axis=-1).astype(np.float32)
    heights = kp[:, :, 1]
...
def features_from_keypoints_torch(keypoints: torch.Tensor, fps: float) -> Tuple[torch.Tensor, torch.Tensor, List[str], List[str]]:
    ...
    cadence_value = 1.0 / max(t * dt, 1e-06)
    cadence = torch.full((b, t), cadence_value, device=keypoints.device, dtype=keypoints.dtype)
    u_names = ['phase_sin', 'phase_cos', 'cadence_hz', 'root_speed_x', 'root_speed_y', 'root_speed_z', 'root_acc_x', 'root_acc_y', 'root_acc_z', 'pelvis_height']
    u = torch.stack([torch.sin(phase).unsqueeze(0).expand(b, t), torch.cos(phase).unsqueeze(0).expand(b, t), cadence, root_v[:, :, 0], root_v[:, :, 1], root_v[:, :, 2], root_a[:, :, 0], root_a[:, :, 1], root_a[:, :, 2], root[:, :, 1]], dim=-1)
