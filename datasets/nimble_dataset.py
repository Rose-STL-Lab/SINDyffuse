...
def read_q_frames(subj: nimble.biomechanics.SubjectOnDisk, trial: int, start_frame: int, num_frames: int, *, kin: int | None=None) -> np.ndarray:
    if kin is None:
        kin = kinematics_pass_index(subj, trial)
    frames = subj.readFrames(trial=trial, startFrame=int(start_frame), numFramesToRead=int(num_frames), includeSensorData=False, includeProcessingPasses=True)
...
def read_q_segment(b3d_path: str, trial: int=0, seg_start: Optional[int]=None, seg_end: Optional[int]=None) -> np.ndarray:
...
def _accumulate_q_stats(cache_dir: Path, ids: list[str], *, max_frames_per_motion: int=0) -> tuple[np.ndarray, np.ndarray, int, int]:
    ...
    for sid in ids:
        ...
        frames = subj.readFrames(trial=trial, startFrame=0, numFramesToRead=n_read, includeSensorData=False, includeProcessingPasses=True)
        for fr in frames:
...
def compute_nimble_normalization_stats(data_root: str | Path, *, split: str='train', splits: Sequence[str] | None=None, max_motions: int=0, max_frames_per_motion: int=0) -> dict:
    ...
    try:
        ...
        sum_q, sum_sq, count, num_dofs = _accumulate_q_stats(cache_dir, cached, max_frames_per_motion=max_frames_per_motion)
        stats_label = 'cache'
    ...
    if meta_path.is_file():
        try:
            ...
            meta = {}
    meta.update({'num_dofs': int(num_dofs), 'stats_split': str(stats_label), 'stats_frame_count': int(count), 'feature_type': 'nimble_q', 'b3d_custom_values': metadata_custom_values_block()})
    meta_path.write_text(json.dumps(meta, indent=2), encoding='utf-8')
    return {'num_dofs': int(num_dofs), 'stats_split': str(stats_label), 'stats_frame_count': int(count), 'mean_path': str(cache_dir / 'Mean.npy'), 'std_path': str(cache_dir / 'Std.npy')}

class NimbleDataset(Dataset):

    def __init__(self, data_root: str, split: str='train', window_size: int=64, fps: int=20, normalize: bool=True, preload: bool=False):
    ...
    def _q_cache_key(self, idx: int) -> _QCacheKey:
        return (self._b3d_paths[idx], self._trials[idx], self._start_frames[idx], self._end_frames[idx])
    ...
    def _append_index(self, motion_id: str, caption: str, start_frame: Optional[int], end_frame: Optional[int], b3d_path: str, trial: int) -> None:
    ...
    def _normalize_window(self, window: np.ndarray) -> np.ndarray:
        if self.normalize and self.mean is not None and (self.std is not None):
            return (window - self.mean) / np.clip(self.std, 1e-08, None)
        return window
    ...
    def __getitem__(self, idx: int):
        ...
        return {'motion': torch.tensor(window, dtype=torch.float32), 'caption': self._captions[idx], 'motion_id': self._motion_ids[idx]}