"""Load HumanML3D 22×3 joint positions from ``joints/`` or ``new_joints/``."""
...
def load_hml3d_joint_positions(
    hml_root: str | Path,
    motion_id: str,
    *,
    joint_source: JointSource = "auto",
    joints_root: str | Path | None = None,
) -> tuple[np.ndarray, str]:
    """Return ``(positions [T,22,3], source_label)``."""
    ...
    # Fallback: decode 263-D vectors (always post-normalization).
    ...
    raise FileNotFoundError(
        f"Could not load 22×3 joints for {motion_id!r} under {root} "
        f"(joint_source={joint_source!r}, joints_root={joints_dir})"
    )


def default_joints_root(hml_root: str | Path) -> Optional[Path]:
    """Optional override via ``HUMANML3D_JOINTS_ROOT`` env var."""
