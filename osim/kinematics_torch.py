"""Torch finite-difference kinematics helpers and spectral summaries."""
...
def finite_diff(x: torch.Tensor, dt: float) -> torch.Tensor:
    """Centered finite difference along time dimension (axis=0)."""
    if x.shape[0] < 2:
...
def fft_energy_ratio(x: torch.Tensor) -> torch.Tensor:
    """Return high-frequency energy ratio as [1, C]."""
    if x.shape[0] < 4:
