...
from common.torch_ops import finite_diff, safe_norm


@dataclass
class BioPhysicsConfig:
    """Lightweight config for bio channel extraction."""
...
def bio_matrix_mint(
    ...
    *,
    fps: float = 20.0,
    guidance_cfg: Optional[BioPhysicsConfig] = None,
) -> np.ndarray:
    """MinT ``q`` ``[T, ndof]`` → L_bio-style channels ``[T, C]`` via finite differences."""
    del guidance_cfg  # reserved for future OpenSim-backed channels
    if q.ndim != 2:
