"""Rajagopal ``q`` → task-space keypoints (no HumanML3D layout)."""
...
# Joint world positions used for guidance / SINDy features.
KEYPOINT_JOINT_NAMES: Tuple[str, ...] = (
    "ground_pelvis",
    "hip_l",
    "hip_r",
    "back",
        ...
        for t in range(t_frames):
            ...
            for k, j_idx in enumerate(joint_indices):
                if j_idx not in joint_cache:
                    joint_cache[j_idx] = sk.getJoint(int(j_idx))
                out[t, k, :] = np.asarray(
                    sk.getJointWorldPositions([joint_cache[j_idx]]),
                    dtype=np.float64,
                ).reshape(3)
...
def body_origins(q: torch.Tensor, body_indices: Tuple[int, int]) -> Tuple[torch.Tensor, torch.Tensor]:
    """Foot body world positions ``[T, 3]`` from ``q`` ``[T, ndof]``."""
