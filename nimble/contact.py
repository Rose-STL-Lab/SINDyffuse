...
def estimate_contact_from_feet(foot_left: torch.Tensor, foot_right: torch.Tensor, com_acc: torch.Tensor, dt: float, mass_kg: float=70.0, g_mps2: float=9.81, height_thresh_m: float=0.06, speed_thresh_mps: float=1.2, gate_sharpness: float=15.0) -> Dict[str, torch.Tensor]:
    ...
    l_contact = _sigmoid_gate(left_height, height_thresh_m, gate_sharpness) * _sigmoid_gate(left_speed_xz, speed_thresh_mps, gate_sharpness)
    r_contact = _sigmoid_gate(right_height, height_thresh_m, gate_sharpness) * _sigmoid_gate(right_speed_xz, speed_thresh_mps, gate_sharpness)
    ...
    denom = torch.clamp(contact_lr[:, 0:1] + contact_lr[:, 1:2], min=0.001)
    f_l = f_l / denom
    f_r = f_r / denom
    ...
    return {'contact_wrench': wrench, 'contact_wrench_l2': wrench_l2, 'contact_active': torch.clamp(wrench_l2 * 10.0, max=1.0), 'summary_contact_ratio': contact_lr.mean(dim=0, keepdim=True), 'inferred_contact_active_lr': contact_lr}