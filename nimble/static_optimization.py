...
from nimble.muscle_activation import MuscleActivationConfig, MuscleActivationResult, _storage_to_array, activation_column_for_muscle, opensim_quiet
...
def _parse_activation_sto(sto_path: Path, muscle_name_list: Sequence[str], frame_times: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
    ...
    for mi, name in enumerate(muscle_name_list):
        ...
        if finite.sum() == 1:
            ...
            t_good = times[finite]
            v_good = col[finite]
            activations[:, mi] = np.interp(frame_times, t_good, v_good, left=float(v_good[0]), right=float(v_good[-1]))
    ...
    for lab, idx in label_to_col.items():
        ...
        finite = np.isfinite(times) & np.isfinite(col)
        if finite.sum() >= 1:
            reserve_series.append(np.interp(frame_times, times[finite], col[finite], left=0.0, right=0.0))
    if reserve_series:
        reserve_max = np.max(np.stack(reserve_series, axis=0), axis=0)
    else:
        reserve_max = np.zeros(n_frames, dtype=np.float64)
    meta: Dict[str, Any] = {'static_reserve_peaks': reserve_peaks, 'static_max_reserve_per_frame': reserve_max.astype(np.float64)}
    return (activations.astype(np.float32), meta)

def run_static_optimization(q: np.ndarray, *, cfg: MuscleActivationConfig, work_dir: Path) -> MuscleActivationResult:
    arr = np.asarray(q, dtype=np.float64)
    t_len = int(arr.shape[0])
    if t_len < 2:
        raise ValueError(f'Need at least 2 frames for static optimization, got {t_len}')
    fps = float(cfg.fps)
    dt = 1.0 / max(fps, 1e-08)
    frame_times = np.arange(t_len, dtype=np.float64) * dt
    t0, t1 = (0.0, (t_len - 1) * dt)
    model_path, muscle_name_list = prepare_rajagopal_activation_model(work_dir, cfg, weld_toe_joints=False)
    ...
    with opensim_quiet(cfg.opensim_log_level):
        ...
        if not act_sto.is_file():
            analysis = osim.StaticOptimization.safeDownCast(runner.getAnalysisSet().get(0))
            storage = analysis.getActivationStorage()
            if storage is None or storage.getSize() < 1:
                raise RuntimeError(f'Static optimization produced no activation storage (work_dir={work_dir})')
            analysis.printResults(_STATIC_TOOL_NAME, str(results_dir))
        if not act_sto.is_file():
            raise RuntimeError(f'Missing static optimization output: {act_sto}')
        activations, parse_meta = _parse_activation_sto(act_sto, muscle_name_list, frame_times)
    meta: Dict[str, Any] = {'activation_method': 'static_optimization', 'repaired_frame_count': 0, 'interpolated_frame_count': 0, 'extrapolated_frame_count': 0, 'static_activation_exponent': float(cfg.static_activation_exponent), 'static_lowpass_cutoff_hz': float(cfg.static_lowpass_cutoff_hz), **{k: v for k, v in parse_meta.items() if k != 'static_max_reserve_per_frame'}}
    return MuscleActivationResult(activations=activations, muscle_names=tuple(muscle_name_list), metadata=meta)