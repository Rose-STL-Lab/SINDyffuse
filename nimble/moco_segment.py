...
def segment_frame_counts(fps: float, *, core_s: float=1.4, buffer_s: float=0.14) -> Tuple[int, int]:
    rate = max(float(fps), 1e-08)
    core = max(1, int(round(float(core_s) * rate)))
    buf = max(0, int(round(float(buffer_s) * rate)))
    return (core, buf)

def plan_moco_segments(t_len: int, fps: float, *, core_s: float=1.4, buffer_s: float=0.14) -> List[SegmentSpec]:
    t_len = int(t_len)
    if t_len < 2:
        return []
    core_frames, buffer_frames = segment_frame_counts(fps, core_s=core_s, buffer_s=buffer_s)
    solve_min = core_frames + 2 * buffer_frames
    if t_len < solve_min:
        return [SegmentSpec(index=0, solve_start=0, solve_end=t_len, core_start=0, core_end=t_len)]
    ...
    while core_start < t_len:
        ...
        specs.append(SegmentSpec(index=seg_idx, solve_start=solve_start, solve_end=solve_end, core_start=core_start, core_end=core_end))
        seg_idx += 1
        core_start = core_end
    return specs
...
def apply_ground_offset_q(q: np.ndarray, sk: Any, cfg: MuscleActivationConfig, *, foot_body_names: Sequence[str]=('calcn_l', 'calcn_r')) -> Tuple[np.ndarray, float]:
    ...
    min_y = foot_contact_point_min_y(sk, q_arr, foot_body_names=foot_body_names, sphere_offset_y_m=offset_y)
    shift = float(target_min_y - min_y)
    if abs(shift) > 1e-09:
        q_arr[:, PELVIS_TY_COL] += shift
    return (q_arr, shift)

def stitch_segment_values(t_len: int, segments: Sequence[SegmentSpec], core_values: Sequence[np.ndarray], *, blend_frames: int, stitch_seams: bool) -> np.ndarray:
    if not segments:
    ...
    for spec, core in zip(segments, core_values):
        ...
        if arr.shape != (spec.core_len, n_cols):
            raise ValueError(f'Segment {spec.index} shape {arr.shape} != ({spec.core_len}, {n_cols})')
...
def stitch_segment_mask(t_len: int, segments: Sequence[SegmentSpec], segment_ok: Sequence[bool]) -> np.ndarray:
...
def run_moco_track_segmented(q: np.ndarray, *, cfg: MuscleActivationConfig, work_dir: Path, skeleton: Any | None=None) -> MuscleActivationResult:
    from nimble.moco_track import _effective_mesh_interval, _solve_moco_track, _track_config, build_rajagopal_coord_mapping, prepare_rajagopal_moco_track_model
    ...
    with opensim_quiet(cfg.opensim_log_level):
        ...
        for spec in segments:
            ...
            activations, solve_ok, solve_meta, _, grf_seg = _solve_moco_track(q_seg, cfg=cfg, solve_dir=seg_dir, moco_model_path=moco_model_path, mapping=mapping, muscle_name_list=names_ref, mesh_interval=mesh_interval)
