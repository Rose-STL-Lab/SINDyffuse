"""Physics penalty channels from Nimble ``q`` (Rajagopal keypoints, no HumanML3D FK)."""
...
def load_model(*, with_geometry: bool = False) -> Any:
    """Load the bundled Rajagopal Nimble skeleton.

    ``with_geometry=True`` is accepted for API symmetry; the bundled Rajagopal
    model already ships with mesh shape nodes attached either way.
    """
    try:
...
def clear_cache() -> None:
    """Clear cached skeleton used by guidance (e.g. after SINDy window collection)."""
...
def _get_guidance_fk_cache() -> _GuidanceFkCache:
    ...
    if _GUIDANCE_FK_CACHE_SLOT is None:
        ...
        _GUIDANCE_FK_CACHE_SLOT = _GuidanceFkCache(
            sk=sk,
            ndof=int(sk.getNumDofs()),
            q_lo=q_lo,
            q_hi=q_hi,
            foot_indices=foot_body_indices(sk),
        )
...
def physics_from_q(
    ...
    *,
    ...
    dt: float,
    fps: float,
    cache: _GuidanceFkCache | None = None,
) -> Dict[str, torch.Tensor]:
    """Biomechanical guidance from Rajagopal ``q`` (keypoint FK only)."""
    if q.ndim != 2:
    ...
    keypoints_sm, _ = apply_pose_smoothing_torch(
        keypoints,
        fps=int(round(fps)),
        sampling_frequency=float(fps),
        smooth_poses=bool(guidance_cfg.smooth_poses),
        smooth_cutoff_hz=float(guidance_cfg.smooth_cutoff_hz),
        smooth_butterworth_order=int(guidance_cfg.smooth_butterworth_order),
    )
...
def physics_from_q_batch(
    ...
    *,
    ...
    """Run ``physics_from_q`` on ``q_batch`` ``[B, T, ndof]``."""
    if q_batch.ndim != 3:
        raise ValueError(f"Expected q_batch [B, T, ndof], got {tuple(q_batch.shape)}")
    return [
        physics_from_q(q_batch[i], guidance_cfg=guidance_cfg, dt=dt, fps=fps, cache=cache)
        for i in range(int(q_batch.shape[0]))
    ]
