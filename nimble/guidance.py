...
from nimble.channels import (
    BIOMECH_COMPONENT_KEYS,
    parse_biomech_weights,
)
...
class NimbleGuidanceWeights:
    """Per-component weights for L_bio (keys match :data:`BIOMECH_COMPONENT_KEYS`)."""
...
class DeterministicNimbleGuidance:
    def __init__(
        ...
        data_root: str,
        fps: float = 20.0,
        weights: NimbleGuidanceWeights | None = None,
        nimble_settings: NimbleGuidanceConfig | None = None,
    ):
        root = Path(data_root).expanduser()
        cache = nimble_b3d_dir(root)
        if not cache.is_dir():
            raise FileNotFoundError(
                f"Nimble guidance requires Nimble B3D cache at {cache}. "
        ...
        if not mean_np.is_file() or not std_np.is_file():
            raise FileNotFoundError(
                f"Nimble guidance needs Mean.npy and Std.npy at {root}. "
                f"Run preprocess_nimble.py first."
            )
    ...
    def _aggregate(self, comp: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for k, v in comp.items():
            rv = _robustify(
                v,
                robust=self.nimble_settings.robust,
                huber_delta=float(self.nimble_settings.huber_delta),
                charbonnier_eps=float(self.nimble_settings.charbonnier_eps),
            )
            out[k] = _reduce_time(
                rv,
                mode=self.nimble_settings.time_reduce,
                cvar_alpha=float(self.nimble_settings.cvar_alpha),
                lse_temperature=float(self.nimble_settings.lse_temperature),
            )
        return out
    ...
    def loss_and_stats(self, motion_norm: torch.Tensor) -> tuple[torch.Tensor, Dict[str, Any]]:
        ...
        comp_list = physics_from_q_batch(
            batch_denorm,
            guidance_cfg=self.nimble_settings,
            dt=float(dt),
            fps=float(self.fps),
        )
        ...
        stats: Dict[str, Any] = {
            f"nimble_{k}": stats_sum[k] * inv_b for k in BIOMECH_COMPONENT_KEYS
        }
...
def _split_nimble_guidance_cfg(nimble_cfg: Dict[str, Any] | None) -> tuple[Dict[str, Any], Dict[str, Any]]:
    """Split ``train.nimble_guidance`` into ``weights`` and ``physics`` dicts."""
    if not nimble_cfg:
        return {}, {}
...
def build_nimble_guidance(
    data_root: str,
    fps: float,
    nimble_cfg: Dict[str, Any] | None = None,
    *,
    window_frames: int | None = None,
) -> DeterministicNimbleGuidance:
    """Construct Nimble guidance from ``train.nimble_guidance``."""
    wcfg, pcfg = _split_nimble_guidance_cfg(nimble_cfg)
    max_frames = _physics_max_frames(pcfg, window_frames)

    return DeterministicNimbleGuidance(
        data_root=data_root,
        fps=float(fps),
        weights=NimbleGuidanceWeights(components=parse_biomech_weights(wcfg)),
        nimble_settings=NimbleGuidanceConfig(
            time_reduce=str(pcfg.get("time_reduce", "mean")),
            robust=str(pcfg.get("robust", "huber")),
            huber_delta=_cfg_float(pcfg, "huber_delta", 10.0),
            charbonnier_eps=_cfg_float(pcfg, "charbonnier_eps", 1e-3),
            cvar_alpha=_cfg_float(pcfg, "cvar_alpha", 0.1),
            lse_temperature=_cfg_float(pcfg, "lse_temperature", 10.0),
            t_weight_schedule=str(pcfg.get("t_weight_schedule", "none")),
            smooth_poses=_cfg_bool(pcfg, "smooth_poses", True),
            smooth_cutoff_hz=_cfg_float(pcfg, "smooth_cutoff_hz", 6.0),
            smooth_butterworth_order=_cfg_int(pcfg, "smooth_butterworth_order", 2),
            max_physics_frames=max_frames,
            mass_kg=_cfg_float(pcfg, "mass_kg", 70.0),
            g_mps2=_cfg_float(pcfg, "g_mps2", 9.81),
            contact_height_thresh_m=_cfg_float(pcfg, "contact_height_thresh_m", 0.06),
            contact_speed_thresh_mps=_cfg_float(pcfg, "contact_speed_thresh_mps", 1.2),
            contact_gate_sharpness=_cfg_float(pcfg, "contact_gate_sharpness", 15.0),
            physics_on_cpu=_cfg_bool(pcfg, "physics_on_cpu", False),
            physics_batch_cap=_cfg_int(pcfg, "physics_batch_cap", 0),
            fk_backend=str(pcfg.get("fk_backend", "torch")).strip().lower(),
        ),
    )
