"""Nimble inverse dynamics torques from Rajagopal ``q`` trajectories."""
...
def compute_joint_torques_numpy(
    q: np.ndarray,
    *,
    sk: Any,
    foot_indices: Tuple[int, int],
    dt: float,
    use_contact_id: bool = True,
) -> Tuple[np.ndarray, np.ndarray]:
    """Return ``(tau [T, ndof], qdot [T, ndof])`` from smoothed ``q`` ``[T, ndof]``."""
    ...
    if use_contact_id and t_len >= 3:
        ...
        try:
            result = sk.getMultipleContactInverseDynamicsOverTime(
                q_cols,
                contact_bodies,
                smoothingWeight=1.0,
                minTorqueWeight=1.0,
                velocityPenalty=_zero_velocity_penalty,
            )
...
class _InverseDynamicsNimble(torch.autograd.Function):
    ...
    def forward(
        ...
        tau_np, qdot_np = compute_joint_torques_numpy(
            q_np, sk=sk, foot_indices=foot_indices, dt=float(dt)
        )
        ctx.sk = sk
    ...
    @staticmethod
    def backward(
        ctx: Any, grad_tau: torch.Tensor, grad_qdot: torch.Tensor
        ...
        eps = 1e-4
        q_np = q.detach().contiguous().double().cpu().numpy()
        base_tau, _ = compute_joint_torques_numpy(
            q_np,
            sk=ctx.sk,
            foot_indices=ctx.foot_indices,
            dt=ctx.dt,
        )
...
def id_symmetry_scalars(tau: torch.Tensor) -> dict[str, torch.Tensor]:
    """|tau_left_group - tau_right_group| L2 per ``ID_SYMMETRY_SPECS``."""
    out: dict[str, torch.Tensor] = {}
...
def id_power_total(tau: torch.Tensor, qdot: torch.Tensor) -> torch.Tensor:
    """Mechanical power ``sum(tau * qdot)`` as ``[T, 1]``."""
