...
class MuscleActivationConfig:
    ...
    moco_max_max_fk_loss: Optional[float] = None
...
def add_muscle_activation_cli_args(parser: argparse.ArgumentParser) -> None:
    ...
    grp.add_argument('--moco_mesh_interval', type=float, default=None, help='Moco mesh interval in seconds (default 0.02, MinT 50 colloc/s).')
    ...
    grp.add_argument('--moco_max_mean_fk_loss', type=float, default=-1.0, help='Max mean_fk_loss for IK/Moco gates; <=0 uses ik_gate_config.json when present.')
    grp.add_argument('--moco_max_max_fk_loss', type=float, default=-1.0, help='Max max_fk_loss for IK/Moco gates; <=0 uses ik_gate_config.json when present.')
    ...
    grp.add_argument('--moco_parallel_segments', type=int, default=None, help='Concurrent Moco segments per motion (default 1 on K8s; MinT uses ~6 on fat nodes).')
    grp.add_argument('--ik_gate_config', default='', help='Path to ik_gate_config.json from calibrate_ik_gates.py.')
