...
_MOCO_SIDE_EFFECT_GLOBS = ('delete_this_to_stop_optimization__*.txt', '*_tracked_states.sto')
...
_MOCO_MOD_OPS: Tuple[str, ...] = ('ModOpReplaceJointsWithWelds(mtp)', 'ModOpIgnoreTendonCompliance', 'ModOpReplaceMusclesWithDeGrooteFregly2016', 'ModOpIgnorePassiveFiberForcesDGF', 'ModOpScaleActiveFiberForceCurveWidthDGF', 'ModOpAddResiduals', 'ModOpAddReserves')
...
class MocoTrackConfig:
    ...
    stiffness_N_per_m: float = 100000.0
    dissipation_Ns_per_m: float = 200.0
    multi_contact: bool = True

def _default_foot_contact_spheres(cfg: MuscleActivationConfig) -> Tuple[FootContactSphereSpec, ...]:
    r = float(cfg.moco_contact_sphere_radius_m)
    y = float(cfg.moco_contact_sphere_offset_y_m)
    calcn = (FootContactSphereSpec('calcn_r', r, offset_y_m=y), FootContactSphereSpec('calcn_l', r, offset_y_m=y))
    if not bool(cfg.moco_multi_contact):
        return calcn
    toe_r = float(cfg.moco_contact_toe_radius_m)
    return calcn + (FootContactSphereSpec('toes_r', toe_r, offset_y_m=y * 0.75, offset_z_m=0.06), FootContactSphereSpec('toes_l', toe_r, offset_y_m=y * 0.75, offset_z_m=0.06))

def _track_config(cfg: MuscleActivationConfig) -> MocoTrackConfig:
    multi = bool(cfg.moco_multi_contact)
    return MocoTrackConfig(contact_spheres=_default_foot_contact_spheres(cfg), stiffness_N_per_m=float(cfg.moco_contact_stiffness), dissipation_Ns_per_m=float(cfg.moco_contact_dissipation), multi_contact=multi)
...
def _base_mesh_interval(cfg: MuscleActivationConfig) -> float:
    ...
    dt = 1.0 / max(float(cfg.fps), 1e-08)
    return min(dt, 0.2)
...
def _effective_mesh_interval(cfg: MuscleActivationConfig, q: np.ndarray | None=None) -> float:
...
def _add_foot_contact(model: osim.Model, track_cfg: MocoTrackConfig) -> None:
    ...
    half_space = osim.ContactHalfSpace(osim.Vec3(0, 0, 0), osim.Vec3(0, 0, -np.pi / 2.0), ground, 'floor')
    ...
    for spec in track_cfg.contact_spheres:
        ...
        sphere = osim.ContactSphere(float(spec.radius_m), osim.Vec3(float(spec.offset_x_m), float(spec.offset_y_m), float(spec.offset_z_m)), body, geom_name)
        model.addContactGeometry(sphere)
        force = osim.SmoothSphereHalfSpaceForce(f'contact_force_{spec.body_name}', sphere, half_space)
        force.set_stiffness(float(track_cfg.stiffness_N_per_m))
        force.set_dissipation(float(track_cfg.dissipation_Ns_per_m))
        model.addForce(force)
    model.finalizeConnections()

def prepare_rajagopal_moco_track_model(work_dir: Path, *, model_path: Path | None=None, track_cfg: MocoTrackConfig | None=None) -> Path:
    if track_cfg is None:
        track_cfg = MocoTrackConfig(contact_spheres=_default_foot_contact_spheres(MuscleActivationConfig()), multi_contact=True)
...
def _trial_time_bounds(t_len: int, fps: float) -> Tuple[float, float]:
    dt = 1.0 / max(float(fps), 1e-08)
    t0 = 0.0
    t1 = max(t0, (t_len - 1) * dt - 1e-06)
    return (t0, t1)

def _build_model_processor_track(moco_model_path: Path, cfg: MuscleActivationConfig) -> osim.ModelProcessor:
    return build_activation_model_processor(moco_model_path, cfg, weld_toe_joints=True)
_muscle_names_from_processor = muscle_names_from_processor

def _parse_moco_activation_storage(storage_path: Path, muscle_name_list: Sequence[str], frame_times: np.ndarray) -> Tuple[np.ndarray, bool]:
    ...
    for mi, name in enumerate(muscle_name_list):
        ...
        if finite.sum() < 2:
            continue
        activations[:, mi] = np.interp(frame_times, sol_times[finite], series[finite], left=np.nan, right=np.nan)
...
def _extract_sim_grf_from_moco_solution(moco_sol: osim.MocoSolution, moco_model_path: Path, frame_times: np.ndarray, track_cfg: MocoTrackConfig) -> np.ndarray:
    n_frames = int(frame_times.shape[0])
...
def _analyze_moco_reserve_controls(moco_sol: osim.MocoSolution, frame_times: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
    ...
    if combined.size == 0:
        ...
        meta = {'max_reserve_fraction': 0.0, 'reserve_control_peaks': {}}
        return (by_time, meta)
    finite = np.isfinite(sol_times) & np.isfinite(combined)
    if finite.sum() < 2:
        by_time = np.full(n_frames, float(np.nanmax(combined)), dtype=np.float64)
    else:
        by_time = np.interp(frame_times, sol_times[finite], combined[finite], left=combined[finite][0], right=combined[finite][-1])
    max_frac = float(np.max(by_time)) if by_time.size else 0.0
    meta = {'max_reserve_fraction': max_frac, 'reserve_control_peaks': reserve_peaks}
    return (by_time, meta)

def _configure_moco_state_tracking_weights(study: osim.MocoStudy, *, cfg: MuscleActivationConfig) -> None:
...
def _configure_moco_track_solver(study: osim.MocoStudy, *, cfg: MuscleActivationConfig) -> None:
    problem = study.updProblem()
    ...
    if bool(cfg.moco_minimize_implicit_aux_derivatives):
        ...
        solver.set_implicit_auxiliary_derivatives_weight(float(cfg.moco_implicit_aux_derivatives_weight))
    _configure_moco_state_tracking_weights(study, cfg=cfg)
    solver.resetProblem(problem)

def _solve_moco_track(q: np.ndarray, *, cfg: MuscleActivationConfig, solve_dir: Path, moco_model_path: Path, mapping: Any, muscle_name_list: Sequence[str], mesh_interval: float) -> Tuple[np.ndarray, bool, Dict[str, Any], Path, np.ndarray]:
    t_len = int(q.shape[0])
    dt = 1.0 / max(float(cfg.fps), 1e-08)
    ...
    track.setModel(mp)
    track.setStatesReference(build_moco_states_table_processor(mot_path, lowpass_hz=float(cfg.moco_reference_lowpass_hz)))
    ...
    solve_meta: Dict[str, Any] = {'num_frames': t_len, 't0': t0, 't1': t1, 'mesh_interval': float(mesh_interval), 'moco_reference_lowpass_hz': float(cfg.moco_reference_lowpass_hz), 'moco_states_speed_tracking_weight': float(cfg.moco_states_speed_tracking_weight), 'moco_aux_coord_tracking_weight': float(cfg.moco_aux_coord_tracking_weight), 'moco_apply_tracked_states_to_guess': bool(cfg.moco_apply_tracked_states_to_guess), 'max_joint_speed_deg_s': _max_joint_speed_deg_s(q, float(cfg.fps))}
