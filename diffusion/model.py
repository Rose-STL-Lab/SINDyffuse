...
class FeedForward(nn.Module):
    ...
    def __init__(self, dim: int, mult: int=4, dropout: float=0.1):
        ...
        self.net = nn.Sequential(nn.Linear(int(dim), inner), nn.GELU(), nn.Dropout(float(dropout)), nn.Linear(inner, int(dim)), nn.Dropout(float(dropout)))
...
class DiffusionTransformer(nn.Module):
    ...
    def __init__(self, input_dim: int=37, model_dim: int=512, num_layers: int=8, num_heads: int=8, dropout: float=0.1, max_seq_len: int=256, clip_dim: int=512):
        super().__init__()
        ...
        self.time_mlp = nn.Sequential(SinusoidalPosEmb(model_dim), nn.Linear(model_dim, model_dim), nn.GELU(), nn.Linear(model_dim, model_dim))
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, text_ctx: Optional[torch.Tensor]=None, text_mask: Optional[torch.Tensor]=None, force_uncond: bool=False) -> torch.Tensor:
        _, tlen, _ = x_t.shape
...
def cosine_beta_schedule(timesteps: int, s: float=0.008) -> torch.Tensor:
    ...
    return torch.clip(betas, 1e-08, 0.999).float()
...
class GaussianDiffusionSchedule:
    ...
    def to(self, device: torch.device) -> 'GaussianDiffusionSchedule':
        for name in ['betas', 'alphas', 'alphas_cumprod', 'alphas_cumprod_prev', 'sqrt_alphas_cumprod', 'sqrt_one_minus_alphas_cumprod', 'posterior_variance']:
