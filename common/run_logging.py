...
class RunLogger:
    ...
    def __init__(self, *, terminal: TextIO, log_file: TextIO | None, progress_enabled: bool=True, shared_log: bool=False, line_prefix: str='') -> None:
...
class DualTqdm:
    ...
    def __init__(self, *, total: int, desc: str='', unit: str='', terminal: TextIO, log_file: TextIO | None=None) -> None:
        self._bars: list[tqdm] = [tqdm(total=total, desc=desc, unit=unit, file=terminal, dynamic_ncols=True)]
        if log_file is not None:
            self._bars.append(tqdm(total=total, desc=desc, unit=unit, file=log_file, ascii=True, dynamic_ncols=False, mininterval=1.0))
...
def dual_tqdm(*, total: int, desc: str='', unit: str='', logger: RunLogger | None=None) -> DualTqdm:
    log = logger if logger is not None else get_run_logger()
    log_file = None if log.shared_log else log.log_file_stream
    return DualTqdm(total=total, desc=desc, unit=unit, terminal=log.terminal, log_file=log_file)
...
def build_run_log_paths(log_dir: str | Path, *, script_name: str, rank: int | None=None, run_id: str | None=None) -> RunLogPaths:
    ...
    return RunLogPaths(log_dir=directory, log_file=log_file, latest_log=directory / f'{latest_stem}.log', latest_exit_code=directory / f'{Path(script_name).stem}.exitcode')

def add_run_log_cli_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument('--log_dir', type=str, default=str(default_log_dir()), help='Directory for timestamped run logs (created if missing).')
    parser.add_argument('--no_run_log', action='store_true', help='Disable verbose file logging under logs/.')
    parser.add_argument('--run_log_id', type=str, default='', help='Shared run id for distributed shards (default: SINDYFFUSE_RUN_LOG_ID env).')
...
@contextmanager
def run_log_session(log_dir: str | Path, *, script_name: str, argv: Optional[list[str]]=None, progress_enabled: bool=True, rank: int | None=None, run_id: str | None=None, line_prefix: str='') -> Iterator[tuple[RunLogPaths, RunLogger]]:
    resolved_run_id = resolve_run_log_id(run_id)
    paths = build_run_log_paths(log_dir, script_name=script_name, rank=rank, run_id=resolved_run_id)
    shared_log = resolved_run_id is not None
    log_fp: TextIO
    if shared_log:
        log_fp = _LockedAppendFile(paths.log_file)
    else:
        log_fp = paths.log_file.open('a', encoding='utf-8', buffering=1)
    prev_stderr = sys.stderr
    logger = RunLogger(terminal=sys.stdout, log_file=log_fp, progress_enabled=progress_enabled, shared_log=shared_log, line_prefix=line_prefix)
    sys.stderr = _TeeStream(prev_stderr, log_fp)
...
def run_logged_main(script_name: str, log_dir: str | Path, fn: Callable[[RunLogger], _T], *, argv: Optional[list[str]]=None, no_run_log: bool=False, progress_enabled: bool=True, rank: int | None=None, run_id: str | None=None, line_prefix: str='') -> _T:
    if no_run_log:
        return fn(null_logger())
    with run_log_session(log_dir, script_name=script_name, argv=argv, progress_enabled=progress_enabled, rank=rank, run_id=run_id, line_prefix=line_prefix) as (paths, logger):
